...
func (r *SQLSwiftRepository) CreateBatch(ctx context.Context, banks []*model.SwiftBank) error {
    ...
    for i := 0; i < totalRows; i += batchSize {
        ...
        sb.WriteString(fmt.Sprintf("INSERT INTO %s (swift_code, hq_swift_base, country_iso_code, bank_name, entity_type, created_at, updated_at) VALUES ", r.tableName()))
...
func (r *SQLSwiftRepository) GetBranchesByHQBase(ctx context.Context, hqBase string) ([]model.SwiftBank, error) {
    query := fmt.Sprintf("SELECT swift_code, bank_name, country_iso_code, entity_type, hq_swift_base, created_at, updated_at FROM %s WHERE hq_swift_base = ? AND entity_type = ?", r.tableName())
...
func (r *SQLSwiftRepository) GetByCountry(ctx context.Context, countryCode string) (*CountrySwiftCodes, error) {
    ...
    query := fmt.Sprintf("SELECT swift_code, bank_name, country_iso_code, entity_type, hq_swift_base, created_at, updated_at FROM %s WHERE country_iso_code = ?", r.tableName())
...
func (r *SQLSwiftRepository) getBankByCode(ctx context.Context, code string) (*model.SwiftBank, error) {
    query := fmt.Sprintf("SELECT swift_code, bank_name, country_iso_code, entity_type, hq_swift_base, created_at, updated_at FROM %s WHERE swift_code = ?", r.tableName())
...
func (r *SQLSwiftRepository) getSampleBankName(ctx context.Context, countryCode string) (string, error) {
    query := fmt.Sprintf("SELECT bank_name FROM %s WHERE country_iso_code = ? LIMIT 1", r.tableName())
    var bankName string
...
func scanBank(scanner interface {
    ...
    var createdAt, updatedAt time.Time // Trino driver returns time.Time directly
